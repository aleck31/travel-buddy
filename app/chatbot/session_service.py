...
class SessionService:
    ...
    CONFIRMATION_KEYWORDS = ["confirm", "yes", "book", "proceed", "go ahead"]
    ...
    def _determine_stage(self, session: ChatSession, message: str = "") -> BookingStage:
        ...
        if current_stage_num == 2 and session.stage_data.flight_info:
            return BookingStage.LOUNGE_RECOMMENDATION

        # Stage 3 -> 4: Lounge info collected
        if current_stage_num == 3 and session.stage_data.lounge_info:
        ...
        if current_stage_num == 5 and session.stage_data.order_info:
